...
SELECT "Zoo"."name",
       ZE."Einnahmen",
       ZAF."FutterAusgaben",
       ZAP."PersonalAusgaben",
       ZE."Einnahmen" - ZAF."FutterAusgaben" - ZAP."PersonalAusgaben" as gewinn
FROM "Zoo"
...
--Essen pro Gehege
SELECT "idEnclosure", sum(FP.amount) AS feed_amount
FROM "Enclosure"
...
SELECT "idEnclosure", round(area / count(A), 2) AS area_per_animal
FROM "Enclosure"
         JOIN "Animal" A on "Enclosure"."idEnclosure" = A.enclosure_id
GROUP BY "idEnclosure"
ORDER BY "idEnclosure";
